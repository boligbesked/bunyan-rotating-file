...
var gzipFile = require("./gzip").gzipFile;
...
function RotatingFileStream(opts) {
  ...
  assert(typeof opts.count === "number", format("RotatingFile; options.count is not a number: %j (%s) in %j", opts.count, typeof opts.count, this));
  assert(opts.count >= 0, format("RotatingFile; options.count is not >= 0: %j in %j", opts.count, this));
  ...
  switch (this.periodScope) {
    ...
    case "h":
      if (this._rotateAt) {
        ...
        rotAt = new Date(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours() + 1).getTime();
      }
      break;
    case "d":
      if (this._rotateAt) {
        ...
        rotAt = new Date(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() + 1).getTime();
      }
      break;
    case "w":
      ...
      if (this._rotateAt) {
        ...
        rotAt = new Date(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() + (8 - d.getUTCDay())).getTime();
      }
      break;
    case "m":
      if (this._rotateAt) {
        rotAt = new Date(d.getUTCFullYear(), d.getUTCMonth() + this.periodNum, 1).getTime();
      } else {
...
RotatingFileStream.prototype._rotate = function _rotate() {
  ...
  function zip() {
    ...
    gzipFile(basePath, basePath + ".gz").on("finish", function (err) {
